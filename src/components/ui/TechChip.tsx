...
export function TechChip({ skill, isSelected, onSelect }: Props) {
  const renderIcon = () => {
    const size = 38
  ...
  const styleVars = {
    '--chip-gradient-from': skill.color,
    '--chip-gradient-to': `${skill.color}00`,
    '--chip-glow-color': `${skill.color}60`,
    '--chip-bg-hover': `${skill.color}10`,
  } as React.CSSProperties

  return (
    <div
      className={`tech-chip${isSelected ? ' tech-chip-selected' : ''}`}
      style={styleVars}
      title={skill.name}
