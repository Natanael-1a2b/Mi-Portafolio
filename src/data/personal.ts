export const personalInfo = {
  name: 'Claudio N. Beltre R.',
  brandInitials: 'CNB',
  phone: '829-856-9592',
  email: 'natanaelbeltre03@gmail.com',
  linkedIn: 'https://www.linkedin.com/in/claudio-natanael-beltre-rosario-38a580327/',
  github: 'https://github.com/Natanael-1a2b',
  whatsapp: 'https://wa.me/18298569592',
  profileImage: '/assets/images/Porfile.jpeg',
  cvPath: '/assets/Curriculum Claudio N. Beltre R..pdf',
  formspreeAction: 'https://formspree.io/f/mnnzwnyd',
}

export const heroContent = {
  title: 'Ingeniería de Software para',
  titleHighlight: 'Soluciones Escalables',
  subtitle: 'Especialista en .NET | Arquitectura Limpia | Full Stack',
  description:
    'Transformo requerimientos complejos en sistemas robustos y eficientes. Mi enfoque combina la solidez técnica del backend con experiencias de usuario intuitivas.',
  typingPhrases: [
    'Ingeniería Backend con .NET & Arquitectura Limpia',
    'Soluciones Full Stack que Escalan',
    'Desarrollo Potenciado por Agentes de IA',
  ],
}

export const aboutContent = {
  paragraphs: [
    'Ingeniero de Software con una fuerte especialización en el ecosistema .NET. Mi experiencia se centra en el diseño y desarrollo de arquitecturas escalables, desde APIs RESTful de alto rendimiento hasta aplicaciones web modernas.',
    'Me caracterizo por un aprendizaje acelerado y una capacidad innata para resolver problemas complejos. No solo escribo código; diseño soluciones que aportan valor real al negocio, priorizando la seguridad, la mantenibilidad y las mejores prácticas de la industria (SOLID, Clean Architecture).',
    'Mi objetivo es seguir construyendo sistemas que marquen la diferencia, colaborando en equipos donde la excelencia técnica sea la norma.',
    'Actualmente exploro el desarrollo con <strong>Agentes de IA</strong>, integrando protocolos como <strong>MCP (Model Context Protocol)</strong> y <strong>Skills</strong> para crear soluciones inteligentes y automatizadas que potencian la productividad y la innovación.',
  ],
}

export const footerContent = {
  description:
    'Desarrollador Full Stack enfocado en crear soluciones modernas, escalables y orientadas a resultados reales.',
  services: ['Desarrollo Web', 'APIs REST', 'Sistemas Administrativos', 'Agentes de IA'],
  ctaTitle: '¿Listo para llevar tu proyecto al siguiente nivel?',
  ctaDescription:
    'Disponible para proyectos freelance, trabajo remoto o colaboración por contrato.',
  copyright: '© 2026 CNB · Desarrollo de Software & Soluciones Digitales.',
}

export const navLinks = [
  { href: '#inicio', label: 'Inicio' },
  { href: '#sobre-mi', label: 'Sobre Mí' },
  { href: '#habilidades', label: 'Habilidades' },
  { href: '#experiencia', label: 'Experiencia' },
  { href: '#proyectos', label: 'Proyectos' },
  { href: '#certificaciones', label: 'Certificaciones' },
  { href: '#contacto', label: 'Contacto' },
]
